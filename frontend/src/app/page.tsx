...
import { isAuthenticated, isClient, isSponsor } from '@/lib/auth';
...
export default function ClientServicesPage() {
  ...
  }>({ isAuth: false, isClientUser: false, isSponsorUser: false });

  useEffect(() => {
    setAuthState({
      isAuth: isAuthenticated(),
      isClientUser: isClient(),
      isSponsorUser: isSponsor(),
    });
  }, []);
        ...
        <div className="max-w-[1199px] mx-auto px-8 h-16 flex items-center justify-between">
          ...
          {authState.isAuth ? (
            <div className="flex items-center gap-3">
              {/* ユーザーアイコン */}
              <Link href="/client" className="w-10 h-10 flex items-center justify-center rounded-lg hover:bg-slate-100 transition-colors">
              ...
              {/* ブリーフケースアイコン（Sponsorのみ表示） */}
              {authState.isSponsorUser && (
                <Link href="/sponsor" className="w-10 h-10 flex items-center justify-center rounded-lg hover:bg-slate-100 transition-colors">
                  <svg className="w-5 h-5 text-slate-600" fill="none" viewBox="0 0 20 20">
            ...
            <div className="flex items-center gap-3">
              ...
              <Link
                href="/auth/login"
                className="h-[42px] px-6 flex items-center justify-center border border-[#cad5e2] rounded-lg text-base text-[#0f172b] hover:bg-slate-50 transition-colors"
              >
                ログイン
              </Link>
              {/* 新規登録ボタン */}
              <Link
                ...
                className="h-[42px] px-6 flex items-center justify-center bg-blue-500 text-white text-base rounded-lg hover:bg-blue-600 transition-colors"
              >
                新規登録
              </Link>
            </div>
